...
def date_ticks(df,interval = 7) :
    ...
    datemin = np.datetime64(df['date'].min())
    datemax = np.datetime64(df['date'].max()) + np.timedelta64(1, 'D')
...
def Basic_Analysis_Roll(state_abbrev, start_date = '2020-03-15', test_date='2020-04-15',ICU_date='2020-05-01'):
    ...
    date_ticks(df=test_df,interval=5)
    color = 'gray'
...
#---------------------------------------------------------------------------------   
    ...
    if ICU_df.inIcuCumulative.notnull().sum() >40 :
        ...
        watermark(loc_x=0.94)
        date_ticks(df=ICU_df,interval=5)
        color = 'tab:blue'
